...
void LLVolumeGeometryManager::rebuildGeom(LLSpatialGroup* group)
{
	...
	U32 max_vertices = ((U32)render_max_vbo_size * 1024) / LLVertexBuffer::calcStride(group->mSpatialPartition->mVertexDataMask);
	static LLCachedControl<S32> render_max_node_size("RenderMaxNodeSize", 8192);
	U32 max_total = ((U32)render_max_node_size * 1024) / LLVertexBuffer::calcStride(group->mSpatialPartition->mVertexDataMask);
	...
	for (LLSpatialGroup::element_iter drawable_iter = group->getData().begin(); drawable_iter != group->getData().end(); ++drawable_iter)
	{
		...
		F32 max_for_this_vol = (vobj->isSculpted()) ? gSavedSettings.getF32("RenderSculptSAThreshold") : gSavedSettings.getF32("RenderVolumeSAThreshold");

		if (vobj->mVolumeSurfaceArea > max_for_this_vol)
		{
			LLPipeline::sVolumeSAFrame += vobj->mVolumeSurfaceArea;
			if(LLPipeline::sVolumeSAFrame > gSavedSettings.getF32("RenderVolumeSAFrameMax"))
			{
				continue;
			}
		}
